...
// APrimaryMulti represents a row from 'schemax.a_primary_multi'.
...
func (apm *APrimaryMulti) Insert(ctx context.Context, db DB) error {
	...
	// insert (manual)
	const sqlstr = `INSERT INTO schemax.a_primary_multi (` +
		`a_key, a_text` +
		`) VALUES (` +
		`?, ?` +
		`)`
...
func (apm *APrimaryMulti) Update(ctx context.Context, db DB) error {
	...
	const sqlstr = `UPDATE schemax.a_primary_multi SET ` +
		`a_text = ? ` +
		`WHERE a_key = ?`
	// run
...
func (apm *APrimaryMulti) Delete(ctx context.Context, db DB) error {
	...
	const sqlstr = `DELETE FROM schemax.a_primary_multi ` +
		`WHERE a_key = ?`
	// run
...
// APrimaryMultiByAKey retrieves a row from 'schemax.a_primary_multi' as a [APrimaryMulti].
//
