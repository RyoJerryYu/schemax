...
// ASequence represents a row from 'schemax.a_sequence'.
...
func (as *ASequence) Insert(ctx context.Context, db DB) error {
	...
	const sqlstr = `INSERT INTO schemax.a_sequence (` +
		`` +
		`) VALUES (` +
		`` +
		`)`
...
func (as *ASequence) Delete(ctx context.Context, db DB) error {
	...
	const sqlstr = `DELETE FROM schemax.a_sequence ` +
		`WHERE a_seq = ?`
	// run
	logf(sqlstr, as.ASeq)
...
// ASequenceByASeq retrieves a row from 'schemax.a_sequence' as a [ASequence].
//
