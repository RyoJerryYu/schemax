...
// AIndexComposite represents a row from 'schemax.a_index_composite'.
type AIndexComposite struct {
	AKey1 sql.NullInt64 `json:"a_key1"` // a_key1
	AKey2 sql.NullInt64 `json:"a_key2"` // a_key2
}

// AIndexCompositeByAKey1AKey2 retrieves a row from 'schemax.a_index_composite' as a [AIndexComposite].
//
...
func AIndexCompositeByAKey1AKey2(ctx context.Context, db DB, aKey1, aKey2 sql.NullInt64) ([]*AIndexComposite, error) {
	...
	const sqlstr = `SELECT ` +
		`a_key1, a_key2 ` +
		`FROM schemax.a_index_composite ` +
