...
// APrimaryComposite represents a row from 'schemax.a_primary_composite'.
...
func (apc *APrimaryComposite) Insert(ctx context.Context, db DB) error {
	...
	// insert (manual)
	const sqlstr = `INSERT INTO schemax.a_primary_composite (` +
		`a_key1, a_key2` +
		`) VALUES (` +
		`?, ?` +
		`)`
...
func (apc *APrimaryComposite) Delete(ctx context.Context, db DB) error {
	...
	const sqlstr = `DELETE FROM schemax.a_primary_composite ` +
...
// APrimaryCompositeByAKey1AKey2 retrieves a row from 'schemax.a_primary_composite' as a [APrimaryComposite].
//
...
func APrimaryCompositeByAKey1AKey2(ctx context.Context, db DB, aKey1, aKey2 int) (*APrimaryComposite, error) {
	...
	const sqlstr = `SELECT ` +
		`a_key1, a_key2 ` +
		`FROM schemax.a_primary_composite ` +
		`WHERE a_key1 = ? AND a_key2 = ?`
	// run
