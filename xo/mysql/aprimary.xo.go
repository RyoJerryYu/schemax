...
// APrimary represents a row from 'schemax.a_primary'.
...
func (ap *APrimary) Insert(ctx context.Context, db DB) error {
	...
	// insert (manual)
	const sqlstr = `INSERT INTO schemax.a_primary (` +
		`a_key` +
		`) VALUES (` +
		`?` +
		`)`
...
func (ap *APrimary) Delete(ctx context.Context, db DB) error {
	...
	const sqlstr = `DELETE FROM schemax.a_primary ` +
		`WHERE a_key = ?`
	// run
	logf(sqlstr, ap.AKey)
	if _, err := db.ExecContext(ctx, sqlstr, ap.AKey); err != nil {
...
// APrimaryByAKey retrieves a row from 'schemax.a_primary' as a [APrimary].
//
