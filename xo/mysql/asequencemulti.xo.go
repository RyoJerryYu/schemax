...
// ASequenceMulti represents a row from 'schemax.a_sequence_multi'.
...
func (asm *ASequenceMulti) Insert(ctx context.Context, db DB) error {
	...
	const sqlstr = `INSERT INTO schemax.a_sequence_multi (` +
		`a_text` +
		`) VALUES (` +
		`?` +
		`)`
...
func (asm *ASequenceMulti) Update(ctx context.Context, db DB) error {
	...
	const sqlstr = `UPDATE schemax.a_sequence_multi SET ` +
		`a_text = ? ` +
		`WHERE a_seq = ?`
	// run
...
func (asm *ASequenceMulti) Delete(ctx context.Context, db DB) error {
	...
	const sqlstr = `DELETE FROM schemax.a_sequence_multi ` +
		`WHERE a_seq = ?`
	// run
...
// ASequenceMultiByASeq retrieves a row from 'schemax.a_sequence_multi' as a [ASequenceMulti].
//
